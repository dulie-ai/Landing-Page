...
This page reuses the current site's actual components and styles, including the phone, benefits, phrasing examples, command guide and technical section. Future planning is appended as a scoped extension, not a replacement. It shares the root dependency installation and deployment. The second build only clears `dist/future`, never the existing site.
...
The user confirmed that all existing functionality remains available. The full current-site feature coverage is retained, including edit/reschedule, undo, recurrence, Google Tasks and morning summaries. No live user account was used to exercise the bot.
